...
def serialconnect():
  global baudrate, ser_controller, controllerPort
  ...
  try :
      ...
      try:
          ser_controller = serial.Serial(controllerPort, baudRate)
          print "Serial port " + controllerPort + " opened  Baudrate " + str(baudRate)
      except SerialException:
          return 0

def serialconnect_sensor():
  global baudrate, ser_sensor, sensorPort
  ...
  try :
      ...
      try:
          ser_sensor = serial.Serial(sensorPort, baudRate)
          print "Serial port " + sensorPort + " opened  Baudrate " + str(baudRate)
      except SerialException:
          return 0
